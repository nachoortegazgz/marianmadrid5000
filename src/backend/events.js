...
CORRECTIONS APPLIED:
  [FIX-1] wixEcom_onOrderCanceled: "const orderId = String(order?._id || order?.id ||  " ")" -> removed trailing space
  [FIX-2] wixEcom_onOrderCanceled: 'if (orderId !==  "unknown ")' -> 'if (orderId !== "unknown")'
  [FIX-3] wixEcom_onOrderCanceled: 'return { status:  "OK " }' -> 'return { status: "OK" }'
*/
...
const orderId = String(order?._id || order?.id || "").trim() || "unknown";  // [FIX-1] removed trailing space
if (orderId !== "unknown") {                                                 // [FIX-2] was 'orderId !==  "unknown "'
const lineItems = Array.isArray(order.lineItems) ? order.lineItems : [];
const bookingsAppId = APP_IDS.BOOKINGS;
...
return { status: "OK" };                                                    // [FIX-3] was 'return { status:  "OK " }'
} catch (error) {
