/*
 ...
 VERSION: v5002.3-reservas-canonical-slugUrl
 RESPONSIBILITY: Availability engine for single and dual bookings, workload-balanced
                 staff allocation, RAM cache management, and service resolution
                 from ServiciosCatalogo.
 CORRECTIONS APPLIED (Matriz Correspondencia + dato del usuario):
   - slugUrl se mantiene como campo canónico (NO se renombra a slug)
   - tituloServicio -> title
   - tipoServicio -> serviceType
   - duracionTotal -> totalDuration
   - tiempoFase1/2 -> phase1Duration/phase2Duration
   - tiempoExposicion -> exposureDuration
   - linkFases -> linkedPhases
   - permitirCombinar -> allowCombine
   - precio -> price
   - moneda -> currency
   - modeloPrecio -> pricingModel
   - depositoValor/Tipo -> depositAmount/depositType
   - pagoOnline/Presencial -> onlinePayment/inPersonPayment
   - impuestoIncluido -> taxIncluded
   - impuestoIva -> taxRate
   - codigoSku -> sku
   - categoriaId/Nombre -> categoryId/categoryName
   - localizacionId -> locationId
   - localizacion -> location
   - staffDisponible -> availableStaff
   - addonsOptions -> addOnOptions
   - tituloAddons -> addOnTitles
   - resumenCorto -> tagLine
   - descripcionLarga -> description
   - imagenPrincipal -> mainMedia
   - oculto -> hidden
   - notasInternas -> internalNotes
   - fechaYmdMadrid -> dateYmd (CitasF2)
   - statusPago -> paymentStatus (CitasF2)
   - tipo -> bookingType (CitasF2)
   - dateYMD -> dateYmd (DualSlotCache)
 ...
 */
 ...
 const log = logger;
 // IDs de colecciones confirmados por el usuario
 const SERVICIOS_COL = COLLECTIONS?.SERVICIOS_CATALOGO || "ServiciosCatalogo";
 const DUAL_CACHE_COL = COLLECTIONS?.DUAL_SLOT_CACHE || "DualSlotCache";
 const DAYS_CACHE_COL = COLLECTIONS?.AVAILABILITY_DAYS_CACHE || "AvailabilityDaysCache";
 const CITAS_COL = COLLECTIONS?.CITAS_F2 || "CitasF2";
 ...
 function _normalizeServiceAddon(addon, fallbackId) {
   ...
   const rawQuantity = addon?.bookingsAddonQuantity ?? addon?.maxQuantity;
   ...
   return {
     ...
     maxQuantity: bookingsAddonQuantity,
   };
 }
 ...
 async function _mapServiceToPresentation(service, traceId) {
   // CORRECCION: serviceId es el campo canonico (no idServicio)
   ...
   // CORRECCION: hidden es el campo canonico (no oculto)
   const isHidden = service?.hidden === true;
   // CORRECCION: linkedPhases es el campo canonico (no linkFases)
   ...
   // CORRECCION: allowCombine es el campo canonico (no permitirCombinar)
   const allowCombine = !isHidden && service?.allowCombine === true && Boolean(secondaryServiceGuid);
   // CORRECCION: phase1Duration, exposureDuration, phase2Duration
   ...
   // CORRECCION: totalDuration es el campo canonico (no duracionTotal)
   const totalDuration = Number(service?.totalDuration) || (phase1Duration + exposureDuration + phase2Duration) || 30;
   // CORRECCION: title es el campo canonico (no tituloServicio)
   const title = _safeTrim(service?.title || "Servicio");
   // CORRECCION: price es el campo canonico (no precio)
   const price = Number(service?.price) || 0;
   // CORRECCION: slugUrl se mantiene como campo canonico (dato del usuario)
   const slugUrl = _safeTrim(service?.slugUrl) || null;
   // CORRECCION: mainMedia es el campo canonico (no imagenPrincipal)
   const imageUrl = _safeTrim(service?.mainMedia || service?.imageUrl) || "";
   // CORRECCION: location es el campo canonico (no localizacion)
   const location = _safeTrim(service?.location) || "Marian Madrid";
   // CORRECCION: tagLine es el campo canonico (no resumenCorto)
   const tagLine = _safeTrim(service?.tagLine) || null;
   // CORRECCION: description es el campo canonico (no descripcionLarga)
   const description = _safeTrim(service?.description) || null;
   let addons = [];
   // CORRECCION: addOnOptions es el campo canonico (no addonsOptions)
   ...
   let staffIds = [];
   try {
     // CORRECCION: availableStaff es el campo canonico (no staffDisponible)
 ...
 export async function _getServiceBySlugOrIdInternal(slugOrId, externalTraceId = null) {
   ...
   try {
     ...
     if (isGuid) {
       ...
       // CORRECCION CRITICA: slugUrl es el campo canonico (dato del usuario)
       // NO se usa slug. Se consulta directamente por slugUrl.
       let res = await withTimeout(
