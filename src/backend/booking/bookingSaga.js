/*
 ...
 VERSION: v5002.3-canonical-saga-aligned
 RESPONSIBILITY: Transactional Saga Orchestrator Engine for single and dual
             booking transactions, distributed locks, rollback recovery,
             and CompensacionesPendientes logging.
 CORRECTIONS APPLIED (Matriz Correspondencia Wix V2):
   - permitirCombinar -> allowCombine (canonical Wix V2 field)
   - tiempoFase1/tiempoFase2 -> phase1Duration/phase2Duration
   - tiempoExposicion -> exposureDuration
   - linkedPhases as primary F2 service source (replaces secondaryServiceGuid)
   - metodoPago/estadoPago -> paymentMethod/paymentStatus (canonical meta)
   - servicioNombre -> title (canonical service name)
   - fechaYmdMadrid -> dateYmd (canonical date field)
   - existingCita check uses meta.paymentStatus (not meta.estadoPago)
   - sagaCompleted flag for post-saga compensation on persistence failure
   - _compensateCreatedBookings exported as standalone function
 ...
 */
 ...
 // D3/D4: INTERNAL FUNCTIONS IMPORTED DIRECTLY (NOT WEB METHODS)
 import {
 ...
 const log = logger;
 const CITAS_COLLECTION = COLLECTIONS?.CITAS_F2 || COLLECTIONS?.CITAS || 'CitasF2';
 const COMPENSATIONS_COLLECTION = COLLECTIONS?.COMPENSACIONES_PENDIENTES || COLLECTIONS?.COMPENSATIONS || 'CompensacionesPendientes';
  ...
  * Uses phaseOneServiceId directly instead of slotF1.serviceId.
  */
 ...
 /**
  * Best-effort compensation for bookings already created in Wix:
  * attempts cancellation and, if cancellation does not confirm,
  * registers a PENDING compensation record for the worker to retry.
  */
 export async function _compensateCreatedBookings(createdBookings, traceId) {
   for (const b of createdBookings) {
     ...
     if (!cancelled) {
       // The pending compensation insert is awaited to avoid losing it
       try {
         await withTimeout(
           wixData.insert(
             COMPENSATIONS_COLLECTION,
             {
               ...
               attempts: 0,
               traceId,
               concept: `Rollback compensation for booking ${b.bookingId} (${b.phase.key})`,
               origin: 'SAGA_COMPENSATION_ROLLBACK',
 ...
 export async function executeBookingSaga(unsafePayload) {
   ...
   try {
     ...
     // MATRIZ FIX: allowCombine is the canonical Wix V2 field (fallback to legacy names)
     const allowsCombine = serviceData.allowCombine === true || serviceData.permiteCombinarFases === true || serviceData.permitirCombinar === true;
     if (isDualRequested && !allowsCombine) {
       return { status: 'ERROR', error: { code: 'DUAL_NOT_ALLOWED', message: 'Este servicio no permite combinacion en dos fases.' } };
     }
     // MATRIZ FIX: phase1Duration / exposureDuration / phase2Duration (canonical Wix V2 fields)
     ...
     // MATRIZ FIX: linkedPhases is the primary F2 service source (canonical Wix V2 field)
     const isDual = !!(
       isDualRequested &&
       allowsCombine &&
     ...
     if (existingCita) {
       ...
       // MATRIZ FIX: uses meta.paymentStatus (canonical) not meta.estadoPago
     ...
     // MATRIZ FIX: title is the canonical service name field
     const serviceName = metaCita?.servicioNombre || serviceData?.metadata?.title || serviceData?.title || 'Servicio';
     // MATRIZ FIX: price is the canonical price field
     const basePrice = Number(metaCita?.precioBase || serviceData?.metadata?.pricing?.base || serviceData?.price || 0);
     const totalBilled = basePrice + addonsTotal;
     // MATRIZ FIX: dateYmd is the canonical date field
     const madridDateYMD = f1LocalStart.slice(0, 10);
     for (const p of phases) {
       // MATRIZ FIX: phase2Duration / phase1Duration (canonical Wix V2 fields)
     ...
     // MATRIZ FIX: paymentMethod is the canonical field (not metodoPago)
     ...
     sagaCompleted = true;
     // MATRIZ FIX: paymentPlan uses canonical paymentMethod and paymentStatus fields
     const paymentPlan = {
     ...
     const persistPromises = createdBookings.map((b) => {
       ...
       return _persistBooking({
         ...
         meta: {
           ...baseMeta,
           // MATRIZ FIX: paymentStatus and paymentMethod are canonical (not estadoPago/metodoPago)
     ...
     // MATRIZ FIX: if failure occurs after saga execution (e.g. during persistence),
     // bookings are already created in Wix and must be compensated
