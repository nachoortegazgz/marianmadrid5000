 ...
 * VERSION: v5001-optimized (base v18.9.2-fix-scope)
 * RESPONSIBILITY: Transactional Saga Orchestrator Engine for simple and dual
 *                 booking transactions.
 * v5001 CHANGELOG (respecto a v18.9.2-fix-scope; IDs/nombres de campo intactos):
 *  - FIX: en el paso de compensacion (rollback) del saga, el insert del
 *    registro PENDING en COMPENSATIONS_COLLECTION ya no es fire-and-forget
 *    (antes: .catch(() => { withTimeout(...).catch(() => {}) }) sin await).
 *    Ahora se espera (await) dentro de un try/catch anidado, evitando que el
 *    registro de compensacion pendiente se pierda si el proceso continua
 *    antes de persistirlo.
 *  - REFACTOR: la query de existingCita (VECTOR A) paso de un
 *    await (...).then(...).catch(...) mixto a async/await + try/catch puro,
 *    sin cambiar el comportamiento (misma logica, mas legible).
...
export async function executeBookingSaga(unsafePayload) {
  ...
  try {
    ...
    lockKeys = _buildLockKeys_DEPRECATED(phases, lockResourceKey);
