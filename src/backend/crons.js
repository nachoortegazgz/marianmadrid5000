...
VERSION: v5001-optimized (base v19.6.15-pending-compensations-recovery)
...
.query(COLLECTIONS.LOCKS)
.lt("expiresAt", now)
.limit(100)
.find({ suppressAuth: true });
let removed = 0;
for (const item of res?.items || []) {
await wixData.remove(COLLECTIONS.LOCKS, item._id, { suppressAuth: true }).catch(() => null);
removed++;
}
...
const res = await wixData
.query(COLLECTIONS.AUDIT_LOG)
.lt("fechaLog", cutoff)
.limit(100)
.find({ suppressAuth: true });
let removed = 0;
for (const item of res?.items || []) {
await wixData.remove(COLLECTIONS.AUDIT_LOG, item._id, { suppressAuth: true }).catch(() => null);
removed++;
}
