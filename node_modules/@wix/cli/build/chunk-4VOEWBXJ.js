...
function reportCommandStartEvent({
  ...
  return {
    succeed(fields) {
      const _msid = fields?.msid ?? msid;
      const _appId = fields?.appId ?? appId;
      void biLogger.report(
...
// ../cli-telemetry/src/create-bi-logger.ts
async function createBiLogger(defaultFields = {}) {
  ...
  return new StandaloneNodeLogger().factory(options).updateDefaults({
    localTime: () => Date.now(),
    ...defaultFieldsToBiEvent(defaultFields)
  }).setMuted(!isTelemetryOn).logger();
}
function defaultFieldsToBiEvent(defaultFields) {
  const { cliEnvName, aiAgentName, userId, cliTrackingId, ...rest } = defaultFields;
  const result = {};
  ...
  if (cliEnvName) {
    result.environment = cliEnvName;
  }
  if (aiAgentName) {
    result.ai_agent_name = aiAgentName;
  }
  if (cliTrackingId) {
    result.tracking_id = cliTrackingId;
  }
  return { ...result, ...resolveUserFields(userId) };
}
...
//# sourceMappingURL=chunk-4VOEWBXJ.js.map