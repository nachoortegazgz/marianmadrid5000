...
import {
  ...
  Trans,
  useTranslation
} from "./chunk-BMJ4435Y.js";
import {
  openBrowser
} from "./chunk-FFLFTAXH.js";
import {
  Alert,
  Box_default,
  ConfirmInput,
...
import {
  ...
  wixCliLoginEnd,
  wixCliLoginStart
} from "./chunk-4VOEWBXJ.js";
import {
  createEnv,
  require_index_node,
  require_src
} from "./chunk-QH7LPHNT.js";
import {
  require_react
} from "./chunk-TODKQSCF.js";
import {
  getDataDirPath
} from "./chunk-MVKKSRVS.js";
import {
  pathExists,
  readJson,
  writeJson
} from "./chunk-S6M6FBNE.js";
import {
  getTestOverrides
...
function parseTraceParent(traceParent) {
  var match12 = TRACE_PARENT_REGEX.exec(traceParent);
  if (!match12)
    return null;
  if (match12[1] === "00" && match12[5])
    return null;
  return {
    traceId: match12[2],
    spanId: match12[3],
    traceFlags: parseInt(match12[4], 16)
  };
}
      ...
      inc(release2, identifier, identifierBase) {
        if (release2.startsWith("pre")) {
          ...
          if (identifier) {
            ...
            const match12 = `-${identifier}`.match(r);
            if (!match12 || match12[1] !== identifier) {
...
var require_coerce = __commonJS({
  "../../node_modules/semver/functions/coerce.js"(exports, module2) {
    ...
    var coerce = (version, options) => {
      ...
      let match12 = null;
      if (!options.rtl) {
        match12 = version.match(options.includePrerelease ? re[t.COERCEFULL] : re[t.COERCE]);
      } else {
        const coerceRtlRegex = options.includePrerelease ? re[t.COERCERTLFULL] : re[t.COERCERTL];
        let next;
        while ((next = coerceRtlRegex.exec(version)) && (!match12 || match12.index + match12[0].length !== version.length)) {
      ...
      const major = match12[2];
      const minor = match12[3] || "0";
      const patch = match12[4] || "0";
      const prerelease = options.includePrerelease && match12[5] ? `-${match12[5]}` : "";
      const build = options.includePrerelease && match12[6] ? `+${match12[6]}` : "";
...
init_esm_shims();
function findByPath(obj, path3, defaultValue, suffix) {
  let result = obj;
  for (const field of path3.split(".")) {
    if (!result) {
      return defaultValue;
    }
    result = result[field];
...
function resolveUrl(opts) {
  ...
  const path3 = injectDataIntoProtoPath(opts.protoPath, opts.data || {});
  return resolvePath(path3, mappings);
}
function injectDataIntoProtoPath(protoPath, data) {
  return protoPath.split("/").map((path3) => maybeProtoPathToData(path3, data)).join("/");
}
...
function resolvePath2(path3, key) {
  return `${path3}${path3 ? "." : ""}${key}`;
}
...
function createHttpClient({
  ...
  const cookieHeader = Object.entries({ "XSRF-TOKEN": "nocheck", ...cookies }).map(([name, value]) => `${name}=${value}`).join("; ");
...
async function logout() {
  ...
  if (!(0, import_variant7.isType)(authData, AccountAuthData.Unauthenticated)) {
...
// ../cli-auth/src/login-bi.ts
init_esm_shims();
function reportLoginStart(biLogger) {
  void biLogger.report(wixCliLoginStart({}));
}
function reportLoginSuccess(biLogger, userInfo) {
  addUserFields(biLogger, userInfo.userId);
  void biLogger.report(wixCliLoginEnd({ isSuccess: true }));
}
function reportLoginFailure(biLogger, error) {
...
var ApiKeyLogin = ({ token: token2, onLogin, onError }) => {
  ...
  const { status } = useAsync(
    async () => {
      reportLoginStart(biLogger);
      return apiKeyAuthStrategy.login({ token: token2 });
    },
    [],
    {
      onSuccess: (authData) => {
        reportLoginSuccess(biLogger, authData.userInfo);
        onLogin(authData);
      },
      onError: (error) => {
        reportLoginFailure(biLogger, error);
        onError(error);
      }
    }
  );
...
var RefreshTokenLogin = ({
  ...
  const { status } = useAsync(
    async () => {
      reportLoginStart(biLogger);
      return unifiedAuthStrategy.loginWithRefreshToken({ refreshToken });
    },
    [],
    {
      onSuccess: (authData) => {
...
// ../cli-auth/src/agent-auth.ts
init_esm_shims();

// ../cli-auth/src/device-code.ts
init_esm_shims();
...
// ../cli-auth/src/agent-auth.ts
function emitLoginFlowMessage(event) {
  console.log(`${JSON.stringify(event)}
`);
}
function createAwaitingUserLoginFlowMessage(deviceCode2) {
  const { coloredVerificationUri } = buildVerificationUris(
    deviceCode2.verificationUri
  );
  return {
    event: "awaiting_user",
    expiresInSeconds: deviceCode2.expiresIn,
    userCode: deviceCode2.userCode,
    verificationUri: coloredVerificationUri
  };
}
function createLoggedInLoginFlowMessage(userInfo) {
  return {
...
function createSuccessfulLoginFlowMessage(userInfo) {
  return {
    email: userInfo.email,
    event: "success",
    userId: userInfo.userId
...
async function runAgentLoginFlow({
  biLogger,
  loginFn = unifiedAuthStrategy.login,
  signal = new AbortController().signal
}) {
  reportLoginStart(biLogger);
  try {
    const deviceCode2 = await requestDeviceCode();
    emitLoginFlowMessage(createAwaitingUserLoginFlowMessage(deviceCode2));
    const authData = await loginFn({
      deviceCode: deviceCode2,
      signal
    });
    reportLoginSuccess(biLogger, authData.userInfo);
    emitLoginFlowMessage(createSuccessfulLoginFlowMessage(authData.userInfo));
    return authData;
  } catch (error) {
    reportLoginFailure(biLogger, error);
    throw error;
...
// ../cli-auth/src/login-mode.ts
init_esm_shims();
var import_react6 = __toESM(require_react(), 1);
var LoginModeContext = (0, import_react6.createContext)(void 0);
var LoginModeProvider = LoginModeContext.Provider;
function useLoginModeContext() {
  return (0, import_react6.useContext)(LoginModeContext);
}
...
`;
    if (error instanceof Error) {
      content += `Error Trace
`;
      content += `${import_verror.default.fullStack(error)}
`;
      content += `Error Info
`;
      content += `${JSON.stringify(import_verror.default.info(error), null, 2)}
`;
...
var DebugLogContext = (0, import_react7.createContext)(null);
var DebugLogProvider = ({ filePath, children }) => {
  const writeToDebugLog = (0, import_react7.useCallback)(
    (error) => writeCrashReport(filePath, error, /* @__PURE__ */ new Date()),
    [filePath]
  );
  return /* @__PURE__ */ import_react7.default.createElement(DebugLogContext.Provider, { value: writeToDebugLog }, children);
};
var useDebugLog = () => {
  const context2 = (0, import_react7.useContext)(DebugLogContext);
  if (context2 == null) {
    throw new Error(
...
function _outdentArray(strings, firstInterpolatedValueSetsIndentationLevel, options) {
  ...
  var match12 = strings[0].match(reDetectIndentation);
  if (match12) {
    indentationLevel = match12[1].length;
  }
  var reSource = "(\\r\\n|\\r|\\n).{0," + indentationLevel + "}";
...
var ErrorMessage = ({ message, cause, explanation, hint }) => {
  ...
  return /* @__PURE__ */ import_react8.default.createElement(
    ErrorDetails,
    {
      message: originalCause ? /* @__PURE__ */ import_react8.default.createElement(import_react8.default.Fragment, null, message, " Cause: ", originalCause.message) : message,
      hint,
      explanation
    }
  );
};
var SystemErrorMessage = ({ message, explanation, hint }) => /* @__PURE__ */ import_react8.default.createElement(
  ErrorDetails,
  {
    message,
    explanation,
...
var OutdatedVersionMessage = ({
  ...
  return /* @__PURE__ */ import_react9.default.createElement(I18nProvider, { messages: messages_default2 }, /* @__PURE__ */ import_react9.default.createElement(Notification, null, deprecated && /* @__PURE__ */ import_react9.default.createElement(
    Trans,
    {
      i18nKey: "version_notifier.deprecated.message",
      components: [/* @__PURE__ */ import_react9.default.createElement(Text, { skin: "secondary" }), /* @__PURE__ */ import_react9.default.createElement(Text, { bold: true })],
      values: {
        currentVersion
      }
    }
    ...
    Trans,
    {
      ...
      components: [/* @__PURE__ */ import_react9.default.createElement(Text, { skin: "secondary" }), /* @__PURE__ */ import_react9.default.createElement(Text, { skin: "success" })],
      values: {
        currentVersion,
        latestVersion
      }
    ...
    Trans,
    {
      ...
      components: [/* @__PURE__ */ import_react9.default.createElement(Text, { skin: "info" })],
      values: {
...
var ServeDeprecationMessage = ({
  ...
  return /* @__PURE__ */ import_react10.default.createElement(Box_default, { rowGap: 1, paddingBottom: 1, flexDirection: "column" }, /* @__PURE__ */ import_react10.default.createElement(Alert, { type: "error" }, "Deprecation Notice"), /* @__PURE__ */ import_react10.default.createElement(Text, null, /* @__PURE__ */ import_react10.default.createElement(Text, { skin: "info" }, "`serve`"), " command is being replaced by", " ", /* @__PURE__ */ import_react10.default.createElement(Text, { skin: "info" }, "`preview`"), " command."), /* @__PURE__ */ import_react10.default.createElement(Text, null, "Run", " ", /* @__PURE__ */ import_react10.default.createElement(Text, { skin: "info" }, packageManagerRunCmd ? `\`${packageManagerRunCmd} wix app preview\`` : "`wix app preview`"), " ", "instead."));
};

// ../cli-error-reporting/src/get-error-component.tsx
function getErrorComponent(code, cause) {
  return (0, import_variant10.match)(code, {
    UnknownFailure: ({ error }) => {
      return () => /* @__PURE__ */ import_react11.default.createElement(import_react11.default.Fragment, null, /* @__PURE__ */ import_react11.default.createElement(Text, { skin: "error" }, "The CLI encountered an unexpected error:"), error instanceof Error ? /* @__PURE__ */ import_react11.default.createElement(Text, { skin: "error" }, error.message) : /* @__PURE__ */ import_react11.default.createElement(Text, null, JSON.stringify(error)));
    },
    FailedToGetDeviceCode: () => {
      return () => /* @__PURE__ */ import_react11.default.createElement(SystemErrorMessage, { message: "An error occurred while logging in: Failed to issue a login device code." });
    },
    FailedToGetAuthToken: () => {
      return () => /* @__PURE__ */ import_react11.default.createElement(SystemErrorMessage, { message: "An error occurred while logging in: Failed to issue refresh token." });
    },
    FailedToRefreshTokenForSite: () => {
      return () => /* @__PURE__ */ import_react11.default.createElement(SystemErrorMessage, { message: "An error occurred while logging in: Failed to issue refresh token for site." });
    },
    ...
    FailedToQueryLocales: () => {
      return () => /* @__PURE__ */ import_react11.default.createElement(SystemErrorMessage, { message: "Failed to pull multilingual data." });
    },
    FailedToQueryPublishedContents: () => {
      return () => /* @__PURE__ */ import_react11.default.createElement(SystemErrorMessage, { message: "Failed to query published contents." });
    },
    ...
    TranslationUnsupportedForProjectType: () => {
      return () => /* @__PURE__ */ import_react11.default.createElement(ErrorMessage, { message: "Translation command only applicable for headless sites." });
    },
    ...
    ConnectUnsupportedForProjectType: () => {
      return () => /* @__PURE__ */ import_react11.default.createElement(ErrorMessage, { message: "The connect command is only available for Site projects." });
    },
    FailedToRenewAuthToken: () => {
      return () => /* @__PURE__ */ import_react11.default.createElement(SystemErrorMessage, { message: "Failed to renew access token." });
    },
    ...
    FailedToFetchPackageJson: () => {
      return () => /* @__PURE__ */ import_react11.default.createElement(SystemErrorMessage, { message: "Failed to fetch package data." });
    },
    ...
    LatestVersionOfCreateAppRequired: ({
      ...
      return () => /* @__PURE__ */ import_react11.default.createElement(
        ErrorMessage,
        {
          message: /* @__PURE__ */ import_react11.default.createElement(import_react11.default.Fragment, null, "Generating a Wix App is only supported using the latest version (", latestVersion, ") of ", /* @__PURE__ */ import_react11.default.createElement(Text, { bold: true }, "@wix/create-app"), " (current:", " ", currentVersion, ")"),
          hint: /* @__PURE__ */ import_react11.default.createElement(Text, null, "Make sure you are using the following command:", /* @__PURE__ */ import_react11.default.createElement(Newline, null), /* @__PURE__ */ import_react11.default.createElement(Text, { bold: true }, updateCommand), /* @__PURE__ */ import_react11.default.createElement(Newline, null), "If the problem persists, try clearing your package manager's cache.")
        }
      );
    },
    EmptyDirectoryRequiredAppGeneration: ({ folder }) => {
      return () => /* @__PURE__ */ import_react11.default.createElement(
        ErrorMessage,
        {
          message: /* @__PURE__ */ import_react11.default.createElement(import_react11.default.Fragment, null, "Generating a Wix App requires an empty directory but", " ", /* @__PURE__ */ import_react11.default.createElement(Text, { bold: true }, folder), " already exists and isn't empty."),
          hint: /* @__PURE__ */ import_react11.default.createElement(Text, null, "Select a different project name or delete the existing", " ", /* @__PURE__ */ import_react11.default.createElement(Text, { bold: true }, folder), " folder.")
        }
      );
    },
    InvalidNewPackageName: ({ packageName, validations }) => {
      return () => /* @__PURE__ */ import_react11.default.createElement(
        ErrorMessage,
        {
          message: /* @__PURE__ */ import_react11.default.createElement(import_react11.default.Fragment, null, "Generating a Wix App requires a valid NPM package name for the generated ", /* @__PURE__ */ import_react11.default.createElement(Text, { bold: true }, "package.json"), " ", /* @__PURE__ */ import_react11.default.createElement(Newline, null), "The name ", /* @__PURE__ */ import_react11.default.createElement(Text, { bold: true }, packageName), " failed the following validations:", /* @__PURE__ */ import_react11.default.createElement(Newline, null), validations.map((validation, id) => /* @__PURE__ */ import_react11.default.createElement(Text, { key: id }, /* @__PURE__ */ import_react11.default.createElement(Newline, null), "- ", validation))),
          hint: `Select a different project name that follows the validations above.`
        }
      );
    },
    SiteSelectorTimedout: () => {
      return () => /* @__PURE__ */ import_react11.default.createElement(ErrorMessage, { message: `Waiting for app installation timed out.` });
    },
    TypeErrorInvalidUrl: () => {
      return () => /* @__PURE__ */ import_react11.default.createElement(ErrorMessage, { message: `Invalid assets URL.` });
    },
    ...
    FailedToGetDeveloperApps: () => {
      return () => /* @__PURE__ */ import_react11.default.createElement(SystemErrorMessage, { message: "Failed to get the list of your applications." });
    },
    FailedToCreateDevCenterApp: () => {
      return () => /* @__PURE__ */ import_react11.default.createElement(SystemErrorMessage, { message: "Failed to register your app in Wix." });
    },
    FailedToCreateDevCenterOAuthApp: () => {
      return () => /* @__PURE__ */ import_react11.default.createElement(SystemErrorMessage, { message: "Failed to register your oauth app in Wix." });
    },
    FailedToUpdateDevCenterOAuthApp: () => {
      return () => /* @__PURE__ */ import_react11.default.createElement(SystemErrorMessage, { message: "Failed to update your oauth app in Wix." });
    },
    FailedToAddPermission: () => {
      return () => /* @__PURE__ */ import_react11.default.createElement(SystemErrorMessage, { message: "Failed to add permission." });
    },
    FailedToGetPlacements: () => {
      return () => /* @__PURE__ */ import_react11.default.createElement(SystemErrorMessage, { message: "Failed to get placements." });
    },
    FailedToCreateTunnel: () => {
      return () => /* @__PURE__ */ import_react11.default.createElement(ErrorMessage, { message: "Failed to create tunnel." });
    },
    FailedToDeleteTunnel: () => {
      return () => /* @__PURE__ */ import_react11.default.createElement(ErrorMessage, { message: "Failed to delete tunnel." });
    },
    FailedToGetBlocksExtensionsData: () => {
      return () => /* @__PURE__ */ import_react11.default.createElement(ErrorMessage, { message: "Failed to get blocks extensions data." });
    },
    FailedToGetBlocksStructure: () => {
      return () => /* @__PURE__ */ import_react11.default.createElement(SystemErrorMessage, { message: "Failed to get blocks structure." });
    },
    FailedToUpdateBlocksAfterSync: () => {
      return () => /* @__PURE__ */ import_react11.default.createElement(ErrorMessage, { message: "Failed updating blocks service after changes occurred." });
    },
    ...
    FailedToUpdateTelemetryConfig: () => {
      return () => /* @__PURE__ */ import_react11.default.createElement(ErrorMessage, { message: "Failed to change telemetry preferences." });
    },
    ...
    FailedToResolveNpmDependencies: () => {
      return () => /* @__PURE__ */ import_react11.default.createElement(SystemErrorMessage, { message: "Failed to resolve NPM dependencies." });
    },
    FailedToInstallPackages: () => {
      return () => /* @__PURE__ */ import_react11.default.createElement(ErrorMessage, { cause, message: "Failed to install NPM packages." });
    },
    ...
    FailedToReadCache: ({ path: path3 }) => {
      return () => /* @__PURE__ */ import_react11.default.createElement(ErrorMessage, { message: `Failed to read cache file at ${path3}.` });
    },
    FailedToWriteCache: ({ path: path3 }) => {
      return () => /* @__PURE__ */ import_react11.default.createElement(ErrorMessage, { message: `Failed to write cache file at ${path3}.` });
    },
    UploadApplicationFailed: () => {
      return () => /* @__PURE__ */ import_react11.default.createElement(ErrorMessage, { message: "Failed to upload the app." });
    },
    BuiltProjectNeeded: () => {
      return () => /* @__PURE__ */ import_react11.default.createElement(ErrorMessage, { message: "Can't find build output. Execute `wix app build` to build your app before continuing." });
    },
    WixUninstallMissingPackageName: () => {
      return () => /* @__PURE__ */ import_react11.default.createElement(ErrorMessage, { message: "Specify a package name." });
    },
    FailedToReadJson: ({ filePath }) => {
      return () => /* @__PURE__ */ import_react11.default.createElement(ErrorMessage, { cause, message: `Failed to read ${filePath}.` });
    },
    ...
    FailedToWriteWixLock: () => {
      return () => /* @__PURE__ */ import_react11.default.createElement(ErrorMessage, { message: `Failed to write 'wix.lock'.` });
    },
    ...
    NoPagesDirectoryFound: ({ srcDir }) => {
      return () => /* @__PURE__ */ import_react11.default.createElement(
        ErrorMessage,
        {
          message: /* @__PURE__ */ import_react11.default.createElement(import_react11.default.Fragment, null, "No ", /* @__PURE__ */ import_react11.default.createElement(Text, { bold: true }, "pages"), " directory was found under", " ", /* @__PURE__ */ import_react11.default.createElement(Text, { bold: true }, srcDir), "."),
          hint: /* @__PURE__ */ import_react11.default.createElement(Text, null, "Consider re-cloning the project repository or creating a", " ", /* @__PURE__ */ import_react11.default.createElement(Text, { bold: true }, "pages"), " directory under ", /* @__PURE__ */ import_react11.default.createElement(Text, { bold: true }, srcDir), ".")
        }
      );
    },
      ...
      return () => /* @__PURE__ */ import_react11.default.createElement(
        ErrorMessage,
        {
          message: defaultOutdent`
            ...
            The following errors were found:
            ${zodError.errors.map(formatZodIssue).join("\n")}`,
          hint: /* @__PURE__ */ import_react11.default.createElement(Text, null, "Try to re-install ", /* @__PURE__ */ import_react11.default.createElement(Text, { bold: true }, "@wix/cli-app"), " dependency.")
        }
      );
    },
    PermissionDenied: () => {
      return () => /* @__PURE__ */ import_react11.default.createElement(ErrorMessage, { message: "Forbidden operation. Missing permissions." });
    },
    ...
    GridAppFilesMaxLengthExceeded: ({ maxSizeViolationsData, filePaths }) => {
      ...
      return () => /* @__PURE__ */ import_react11.default.createElement(ErrorMessage, { message: errorMessage });
    },
    ...
    FailedToWatchFiles: () => {
      return () => /* @__PURE__ */ import_react11.default.createElement(ErrorMessage, { cause, message: "Failed to watch files." });
    },
    ...
    DeviceCodeTimedOut: () => {
      return () => /* @__PURE__ */ import_react11.default.createElement(ErrorMessage, { message: "The device code wasn't approved in the time allocated. Start the login process again." });
    },
    DuplicateComponentIdsError: ({ issues }) => {
      const Issue = ({ issue }) => {
        const typeToName = (type) => (0, import_variant10.match)(type, {
          page: () => "page",
          plugin: () => "plugin",
        ...
        return /* @__PURE__ */ import_react11.default.createElement(Box_default, { flexDirection: "column" }, /* @__PURE__ */ import_react11.default.createElement(Text, null, /* @__PURE__ */ import_react11.default.createElement(Text, { bold: true }, "Duplicate ID: "), issue.id), /* @__PURE__ */ import_react11.default.createElement(Text, { bold: true }, "Extensions:"), issue.components.map(({ name, path: path3, type }) => /* @__PURE__ */ import_react11.default.createElement(Text, { key: path3 }, "[", typeToName(type), "] ", name, " (", path3, ")")), /* @__PURE__ */ import_react11.default.createElement(Text, null, /* @__PURE__ */ import_react11.default.createElement(Text, { bold: true }, "Possible GUID: "), issue.suggestedId));
      };
      return () => /* @__PURE__ */ import_react11.default.createElement(Box_default, { flexDirection: "column", rowGap: 1 }, /* @__PURE__ */ import_react11.default.createElement(
        ErrorMessage,
        {
      ...
      ), issues.map((issue) => /* @__PURE__ */ import_react11.default.createElement(Issue, { issue, key: issue.id })));
    },
    InvalidResponseData: () => {
      return () => /* @__PURE__ */ import_react11.default.createElement(SystemErrorMessage, { message: "Got unexpected response from server." });
    },
    InvalidCreateAppResponse: () => {
      return () => /* @__PURE__ */ import_react11.default.createElement(ErrorMessage, { message: "`createApp` returned an invalid response." });
    },
    InvalidDeployAppResponse: () => {
      return () => /* @__PURE__ */ import_react11.default.createElement(ErrorMessage, { message: "Got unexpected response from server. Make sure you are running a supported version of the Velo CLI." });
    },
    InvalidGetDeploymentResponse: () => {
      return () => /* @__PURE__ */ import_react11.default.createElement(ErrorMessage, { message: "Got unexpected response from server. Try again later." });
    },
    InvalidGetDeploymentPipelineDescriptionResponse: () => {
      return () => /* @__PURE__ */ import_react11.default.createElement(SystemErrorMessage, { message: "Got unexpected response from server." });
    },
    FailedToGetPagesDiff: () => {
      return () => /* @__PURE__ */ import_react11.default.createElement(ErrorMessage, { message: "Failed to request pages diff." });
    },
    InvalidViteBuildOutput: () => {
      return () => /* @__PURE__ */ import_react11.default.createElement(ErrorMessage, { message: "Invalid vite build output." });
    },
    ViteModuleBundleNotFound: () => {
      return () => /* @__PURE__ */ import_react11.default.createElement(ErrorMessage, { message: "Could not find module bundle in vite build output." });
    },
    ...
    FailedToCreateBackendDeployment: () => {
      return () => /* @__PURE__ */ import_react11.default.createElement(ErrorMessage, { message: "Failed to create backend deployment." });
    },
    FailedToUpdateBackendDeployment: () => {
      return () => /* @__PURE__ */ import_react11.default.createElement(ErrorMessage, { message: "Failed to update backend deployment." });
    },
    FailedToGetBackendDeployment: () => {
      return () => /* @__PURE__ */ import_react11.default.createElement(ErrorMessage, { message: "Failed to get backend deployment." });
    },
    FailedToDeployBackend: () => {
      return () => /* @__PURE__ */ import_react11.default.createElement(ErrorMessage, { message: "Failed to deploy backend." });
    },
    FailedToGetUserInfo: () => {
      return () => /* @__PURE__ */ import_react11.default.createElement(ErrorMessage, { message: "Failed to get user information." });
    },
    FailedToGetMyAccount: () => {
      return () => /* @__PURE__ */ import_react11.default.createElement(SystemErrorMessage, { message: "Failed to get account information." });
    },
    FailedToCreateDevelopmentSite: () => {
      return () => /* @__PURE__ */ import_react11.default.createElement(ErrorMessage, { message: "Couldn't create a development site." });
    },
    FailedToCreateMetaSiteFromTemplate: () => {
      return () => /* @__PURE__ */ import_react11.default.createElement(ErrorMessage, { message: "Failed to create a metasite from template." });
    },
    FailedToGetDevelopmentSites: () => {
      return () => /* @__PURE__ */ import_react11.default.createElement(ErrorMessage, { message: "Failed to request development sites." });
    },
    FailedToGetDevelopmentSitesLimit: () => {
      return () => /* @__PURE__ */ import_react11.default.createElement(ErrorMessage, { message: "Failed to request development sites limit." });
    },
    FailedToGetRequiredApps: () => {
      return () => /* @__PURE__ */ import_react11.default.createElement(SystemErrorMessage, { message: "Failed to get app requirement info." });
    },
    FailedToGetMarketListing: () => {
      return () => /* @__PURE__ */ import_react11.default.createElement(ErrorMessage, { message: "Failed to get market listing." });
    },
    FailedToAddRequiredApp: () => {
      return () => /* @__PURE__ */ import_react11.default.createElement(ErrorMessage, { message: "Failed to add required app." });
    },
    FailedToQuerySites: () => {
      return () => /* @__PURE__ */ import_react11.default.createElement(ErrorMessage, { message: "Failed to request sites list." });
    },
    FailedToGetDomainSuggestions: () => {
      return () => /* @__PURE__ */ import_react11.default.createElement(ErrorMessage, { message: "Failed to get domain suggestions." });
    },
    FailedToGetSite: () => {
      return () => /* @__PURE__ */ import_react11.default.createElement(ErrorMessage, { message: "Couldn't get information about your configured development site." });
    },
    FailedToDeploySite: () => {
      return () => /* @__PURE__ */ import_react11.default.createElement(ErrorMessage, { message: "Failed to deploy site." });
    },
    FailedToDeploySitePreview: () => {
      return () => /* @__PURE__ */ import_react11.default.createElement(SystemErrorMessage, { message: "Failed to deploy site preview." });
    },
    FailedToPublishDeployment: () => {
      return () => /* @__PURE__ */ import_react11.default.createElement(SystemErrorMessage, { message: "Failed to publish site." });
    },
    ...
    DevelopmentSiteNotConfigured: () => {
      return () => /* @__PURE__ */ import_react11.default.createElement(ErrorMessage, { message: "Please select a development site first using the 'wix dev-site' command." });
    },
    ...
    FailedToBuildApp: () => {
      return () => /* @__PURE__ */ import_react11.default.createElement(
        ErrorMessage,
        {
          message: /* @__PURE__ */ import_react11.default.createElement(import_react11.default.Fragment, null, "Failed to build the app.", /* @__PURE__ */ import_react11.default.createElement(Newline, null), cause?.message),
    ...
    FailedToInitializeGitRepo: () => {
      return () => /* @__PURE__ */ import_react11.default.createElement(ErrorMessage, { cause, message: "Failed to initialize git repo." });
    },
    FailedToInstallDependencies: () => {
      return () => /* @__PURE__ */ import_react11.default.createElement(ErrorMessage, { cause, message: "Failed to install dependencies." });
    },
    FailedToCommitChanges: () => {
      return () => /* @__PURE__ */ import_react11.default.createElement(ErrorMessage, { cause, message: "Failed to commit changes." });
    },
    FailedToDeleteAuthData: () => {
      return () => /* @__PURE__ */ import_react11.default.createElement(ErrorMessage, { cause, message: "Failed to delete auth data." });
    },
    ...
    FailedToSaveAuthData: () => {
      return () => /* @__PURE__ */ import_react11.default.createElement(ErrorMessage, { cause, message: "Failed to save auth data." });
    },
    ...
    LocalTemplatePathNotFound: ({ path: path3 }) => {
      return () => /* @__PURE__ */ import_react11.default.createElement(ErrorMessage, { message: `Local template path not found: ${path3}.` });
    },
    FailedToCloneRepo: () => {
      return () => /* @__PURE__ */ import_react11.default.createElement(
        ErrorMessage,
    ...
    InvalidGitUrl: ({ url }) => {
      return () => /* @__PURE__ */ import_react11.default.createElement(SystemErrorMessage, { message: `Invalid Git URL: ${url}.` });
    },
    AuthenticationRequired: () => {
      return () => /* @__PURE__ */ import_react11.default.createElement(ErrorMessage, { message: "You have to already be logged in to run this command." });
    },
    LoginRequired: () => {
      return () => /* @__PURE__ */ import_react11.default.createElement(ErrorMessage, { message: "You are not authenticated, log in by running `wix login`." });
    },
    InvalidApiKey: () => {
      return () => /* @__PURE__ */ import_react11.default.createElement(ErrorMessage, { message: "You used invalid API key to log in." });
    },
    AccountWasDeleted: () => {
      return () => /* @__PURE__ */ import_react11.default.createElement(ErrorMessage, { message: "The account you are trying to log in to has been deleted." });
    },
    SiteConfiguredRequired: () => {
      return () => /* @__PURE__ */ import_react11.default.createElement(ErrorMessage, { message: "You must enter a `--site` in the command options." });
    },
    FailedAddingPackageToWorkspace: () => {
      return () => /* @__PURE__ */ import_react11.default.createElement(
        ErrorMessage,
        {
          ...
          hint: /* @__PURE__ */ import_react11.default.createElement(Text, null, "Full error can be found in ", /* @__PURE__ */ import_react11.default.createElement(Text, { bold: true }, "wix-error.log"), ".")
        }
      );
    },
    FailedToGenerateSelfSignedCertificate: () => {
      return () => /* @__PURE__ */ import_react11.default.createElement(ErrorMessage, { message: "Failed to generate a self-signed certificate for development." });
    },
    NonInteractiveTerminal: () => {
      return () => /* @__PURE__ */ import_react11.default.createElement(ErrorMessage, { message: "This command is not supported in non-interactive terminals. Run with --help to see flags for non-interactive usage, or use an interactive terminal (TTY)." });
    },
    FailedToResolveViteOrigin: () => {
      return () => /* @__PURE__ */ import_react11.default.createElement(ErrorMessage, { message: "Failed to resolve the base url for CSS asset urls." });
    },
    FailedToCreateAppRelease: () => {
      return () => /* @__PURE__ */ import_react11.default.createElement(SystemErrorMessage, { message: "Failed to create an app configuration." });
    },
    FailedToParseHttpErrorResponse: () => {
      return () => /* @__PURE__ */ import_react11.default.createElement(SystemErrorMessage, { message: "Unexpected response from server." });
    },
    FailedToReleaseHeadlessProject: () => {
      return () => /* @__PURE__ */ import_react11.default.createElement(SystemErrorMessage, { message: "Failed releasing a headless site and app." });
    },
    AppReleaseConfigViolations: () => {
      return () => /* @__PURE__ */ import_react11.default.createElement(
        ErrorMessage,
        {
          ...
          hint: /* @__PURE__ */ import_react11.default.createElement(Text, null, "It may be temporary error, try again shortly. If it fails again, reach us on Discord: ", /* @__PURE__ */ import_react11.default.createElement(Link, { url: "https://discord.gg/devs-on-wix" }))
        }
      );
    },
    FailedToUpdateAppRelease: () => {
      return () => /* @__PURE__ */ import_react11.default.createElement(SystemErrorMessage, { message: "Failed to update an app configuration." });
    },
    FailedToUpdateOverrideWithNoSideEffects: () => {
      return () => /* @__PURE__ */ import_react11.default.createElement(SystemErrorMessage, { message: "Failed to update the app preview." });
    },
    FailedToCreateComponents: () => {
      return () => /* @__PURE__ */ import_react11.default.createElement(SystemErrorMessage, { message: "Failed to create components." });
    },
    FailedToFetchPackageDetails: () => {
      return () => /* @__PURE__ */ import_react11.default.createElement(SystemErrorMessage, { message: "Failed to fetch cli package details." });
    },
    FailedToGetResolveNpmDependenciesResult: () => {
      return () => /* @__PURE__ */ import_react11.default.createElement(SystemErrorMessage, { message: `Failed to resolve "wix.lock" file.` });
    },
    CliAppVersionMismatch: ({ cliVersion, cliAppVersion }) => {
      return () => /* @__PURE__ */ import_react11.default.createElement(
        ErrorMessage,
        {
          message: /* @__PURE__ */ import_react11.default.createElement(Text, null, "The versions of ", /* @__PURE__ */ import_react11.default.createElement(Text, { bold: true }, "@wix/cli"), " (", cliVersion, ") and", " ", /* @__PURE__ */ import_react11.default.createElement(Text, { bold: true }, "@wix/cli-app"), " (", cliAppVersion, ") dependencies do not match"),
          hint: /* @__PURE__ */ import_react11.default.createElement(Text, null, "Align their versions in your ", /* @__PURE__ */ import_react11.default.createElement(Text, { bold: true }, "package.json"), " and re-install dependencies.")
        }
      );
    },
    ...
    InvalidCustomElementTagName: ({ tagName, errorMessage }) => {
      return () => /* @__PURE__ */ import_react11.default.createElement(
        ErrorMessage,
        {
          message: /* @__PURE__ */ import_react11.default.createElement(Text, null, "The folder name is used as tagName for your custom element. Unfortunately, ", /* @__PURE__ */ import_react11.default.createElement(Text, { bold: true }, tagName), " is invalid tagName. Reason: ", errorMessage),
          hint: /* @__PURE__ */ import_react11.default.createElement(Text, null, "You can find rules for tagName here:", " ", /* @__PURE__ */ import_react11.default.createElement(Text, { bold: true }, "https://developer.mozilla.org/en-US/docs/Web/API/CustomElementRegistry/define#valid_custom_element_names"))
        }
      );
    },
    InvalidSiteFrontendPackageVersion: ({ packageName, versionRange }) => {
      return () => /* @__PURE__ */ import_react11.default.createElement(
        ErrorMessage,
        {
          message: /* @__PURE__ */ import_react11.default.createElement(Text, null, "Package ", /* @__PURE__ */ import_react11.default.createElement(Text, { bold: true }, packageName), " has an invalid version range ", /* @__PURE__ */ import_react11.default.createElement(Text, { bold: true }, versionRange), " in your", " ", /* @__PURE__ */ import_react11.default.createElement(Text, { bold: true }, "package.json"), " file."),
          hint: /* @__PURE__ */ import_react11.default.createElement(Text, null, "The version range must follow", " ", /* @__PURE__ */ import_react11.default.createElement(Text, { bold: true }, "semantic versioning"), " rules, and it must contain a major version.", /* @__PURE__ */ import_react11.default.createElement(Newline, null), "Read more about it here:", " ", /* @__PURE__ */ import_react11.default.createElement(Link, { url: "https://docs.npmjs.com/about-semantic-versioning" }))
        }
      );
    },
    FailedToCreateSiteOverride: () => {
      return () => /* @__PURE__ */ import_react11.default.createElement(SystemErrorMessage, { message: "Failed to create site override configuration." });
    },
    FailedToGetDeploymentPipelineDescription: () => {
      return () => /* @__PURE__ */ import_react11.default.createElement(SystemErrorMessage, { message: "Failed to get deployment pipeline details." });
    },
    FailedToGetDeploymentStatus: () => {
      return () => /* @__PURE__ */ import_react11.default.createElement(SystemErrorMessage, { message: "Failed to get deployment status." });
    },
    NetworkError: () => {
      return () => /* @__PURE__ */ import_react11.default.createElement(
        ErrorMessage,
    ...
    FailedToGenerateAppMarketQueryParam: () => {
      return () => /* @__PURE__ */ import_react11.default.createElement(SystemErrorMessage, { message: "Failed to get generate appMarket query parameter." });
    },
    FailedToParseLog: () => {
      return () => /* @__PURE__ */ import_react11.default.createElement(SystemErrorMessage, { message: "Failed to parse log from the server." });
    },
    ...
    FailedToFetchAppForVersion: () => {
      return () => /* @__PURE__ */ import_react11.default.createElement(SystemErrorMessage, { message: `Failed to fetch app configuration.` });
    },
    ...
    FailedToConnectToLogsService: () => {
      return () => /* @__PURE__ */ import_react11.default.createElement(ErrorMessage, { message: `Failed to connect to logs service.` });
    },
    ...
    LogsConnectionTimeout: () => {
      return () => /* @__PURE__ */ import_react11.default.createElement(ErrorMessage, { message: `Log session has been closed due to timeout.` });
    },
    ...
    CliAppTemplateNotFound: ({ id }) => {
      return () => /* @__PURE__ */ import_react11.default.createElement(
        ErrorMessage,
        {
          message: /* @__PURE__ */ import_react11.default.createElement(import_react11.default.Fragment, null, "Template with ID ", /* @__PURE__ */ import_react11.default.createElement(Text, { bold: true }, id), " not found."),
    ...
    FailedToQueryApps: () => {
      return () => /* @__PURE__ */ import_react11.default.createElement(SystemErrorMessage, { message: "Failed to query apps." });
    },
    FailedToGetSiteInstalledApps: () => {
      return () => /* @__PURE__ */ import_react11.default.createElement(SystemErrorMessage, { message: "Failed to get site installed apps." });
    },
    FailedToGetSiteInstalledAppInstanceId: () => {
      return () => /* @__PURE__ */ import_react11.default.createElement(SystemErrorMessage, { message: "Failed to get site installed app instance ID." });
    },
    FailedToGetClientSpecMap: () => {
      return () => /* @__PURE__ */ import_react11.default.createElement(SystemErrorMessage, { message: "Failed to get site client spec map." });
    },
    ...
    FailedToCreateVeloApp: () => {
      return () => /* @__PURE__ */ import_react11.default.createElement(SystemErrorMessage, { message: "Failed to create an isolated environment." });
    },
    FailedToUpdateVeloAppFiles: () => {
      return () => /* @__PURE__ */ import_react11.default.createElement(SystemErrorMessage, { message: "Failed to update files in the isolated environment." });
    },
    FailedToGetFilesFromVelo: () => {
      return () => /* @__PURE__ */ import_react11.default.createElement(SystemErrorMessage, { message: "Failed to get initial project files." });
    },
    FailedToOpenBrowser: () => {
      return () => /* @__PURE__ */ import_react11.default.createElement(
        ErrorMessage,
    ...
    FailedToSyncPagesTypes: () => {
      return () => /* @__PURE__ */ import_react11.default.createElement(SystemErrorMessage, { message: "Failed to sync pages types." });
    },
    ...
    ServeDeprecated: ({ packageManagerRunCmd }) => {
      return () => /* @__PURE__ */ import_react11.default.createElement(ServeDeprecationMessage, { packageManagerRunCmd });
    },
    ...
    FailedToUpdateIntellisenseAdapterTypes: () => {
      return () => /* @__PURE__ */ import_react11.default.createElement(ErrorMessage, { cause, message: "Failed to update site types." });
    },
    ...
    FailedToCreateDraftVersion: () => {
      return () => /* @__PURE__ */ import_react11.default.createElement(SystemErrorMessage, { message: "Failed to retrieve release options available for your app." });
    },
    FailedToReleaseDraftVersion: () => {
      return () => /* @__PURE__ */ import_react11.default.createElement(SystemErrorMessage, { message: "Failed to release a new version of your app." });
    },
    ...
    FailedToDeployDocument: () => {
      return () => /* @__PURE__ */ import_react11.default.createElement(SystemErrorMessage, { message: "Failed to deploy site document." });
    },
...
${errorMessage}`
    ...
    DeploymentPipelineFailed: ({ tasksDescriptions }) => {
      return () => /* @__PURE__ */ import_react11.default.createElement(import_react11.default.Fragment, null, /* @__PURE__ */ import_react11.default.createElement(
      ...
      ), /* @__PURE__ */ import_react11.default.createElement(Box_default, { paddingLeft: 1, paddingTop: 1, flexDirection: "column" }, tasksDescriptions.map((task, index) => /* @__PURE__ */ import_react11.default.createElement(import_react11.Fragment, { key: index }, /* @__PURE__ */ import_react11.default.createElement(Alert, { type: task.status === "SUCCESS" ? "success" : "error" }, "[", index + 1, "/", tasksDescriptions.length, "] ", task.taskName, "..."), Object.entries(task.description).map(([path3, content]) => /* @__PURE__ */ import_react11.default.createElement(
        Box_default,
        {
          key: path3,
          paddingLeft: 2,
        ...
        /* @__PURE__ */ import_react11.default.createElement(Text, { bold: true }, path3),
        /* @__PURE__ */ import_react11.default.createElement(Text, null, content.trim())
      ))))));
    },
    FailedToFetchWixLockFile: () => {
      return () => /* @__PURE__ */ import_react11.default.createElement(SystemErrorMessage, { message: `Failed to fetch "wix.lock" file.` });
    },
    ...
    HeadlessArgumentIsInvalid: ({ errorMessage }) => {
      return () => /* @__PURE__ */ import_react11.default.createElement(
        ErrorMessage,
        {
          message: errorMessage,
    ...
    FailedToImportCliApp: () => {
      return () => /* @__PURE__ */ import_react11.default.createElement(ErrorMessage, { cause, message: "Failed to load `@wix/cli-app`." });
    },
    ...
    FailedToGetAppEnvironmentVariablesByEnvironment: () => {
      return () => /* @__PURE__ */ import_react11.default.createElement(SystemErrorMessage, { message: "Failed to pull environment variables." });
    },
    FailedToBulkUpsertAppEnvironmentVariables: () => {
      return () => /* @__PURE__ */ import_react11.default.createElement(SystemErrorMessage, { message: "Failed to set/update environment variable." });
    },
    FailedToDeleteAppEnvironmentVariableByEnvironmentAndKey: () => {
      return () => /* @__PURE__ */ import_react11.default.createElement(SystemErrorMessage, { message: "Failed to remove environment variable." });
    },
    FailedToUploadStaticFiles: () => {
      return () => /* @__PURE__ */ import_react11.default.createElement(SystemErrorMessage, { message: "Failed to upload static files." });
    },
    FailedCreatingAppProject: () => {
      return () => /* @__PURE__ */ import_react11.default.createElement(SystemErrorMessage, { message: "An error occurred while creating an app project." });
    },
    FailedToGetAppProject: () => {
      return () => /* @__PURE__ */ import_react11.default.createElement(SystemErrorMessage, { message: "An error occurred while getting an app project." });
    },
    FailedToCreateCodestoreProject: () => {
      return () => /* @__PURE__ */ import_react11.default.createElement(SystemErrorMessage, { message: "An error occurred while creating a codestore project." });
    },
    FailedCreatingAppDeployment: () => {
      return () => /* @__PURE__ */ import_react11.default.createElement(SystemErrorMessage, { message: "An error occoured while creating an app deployment." });
    },
    FailedFinalizingAppDeployment: () => {
      return () => /* @__PURE__ */ import_react11.default.createElement(SystemErrorMessage, { message: "An error occoured while completing app deployment." });
    },
    FailedCreatingNewFunnelProject: () => {
      return () => /* @__PURE__ */ import_react11.default.createElement(SystemErrorMessage, { message: "An error occurred while creating a new business." });
    },
    ...
    SiteOutputDirectoryOutsideProject: () => {
      return () => /* @__PURE__ */ import_react11.default.createElement(ErrorMessage, { message: "Your site output directory must be located inside the main project folder. Please update your configuration." });
    },
    ...
    FailedToFindAstroConfig: () => {
      return () => /* @__PURE__ */ import_react11.default.createElement(ErrorMessage, { message: "No valid configuration file found. Ensure that an astro.config.{js,cjs,mjs,ts} file exists in your project directory." });
    },
    ProjectIsAlreadyLinked: () => {
      return () => /* @__PURE__ */ import_react11.default.createElement(ErrorMessage, { message: "This project is already connected to Wix, so no changes were made." });
    },
    ProjectIsNotLinkable: () => {
      return () => /* @__PURE__ */ import_react11.default.createElement(ErrorMessage, { message: "Only Astro projects can be linked to Wix. The current project is missing an Astro configuration file." });
    },
    ...
    FailedToCreateShareUrl: () => {
      return () => /* @__PURE__ */ import_react11.default.createElement(SystemErrorMessage, { message: "Failed to create preview URL." });
    },
    FailedToListShareUrls: () => {
      return () => /* @__PURE__ */ import_react11.default.createElement(SystemErrorMessage, { message: "Failed to list preview URLs." });
    },
    ...
    FailedToDistributeClientManifestUpdate: () => {
      return () => /* @__PURE__ */ import_react11.default.createElement(SystemErrorMessage, { message: "Failed to distribute client manifest update." });
    },
    FailedToCreateRoutesWithMetaSiteUrl: () => {
      return () => /* @__PURE__ */ import_react11.default.createElement(SystemErrorMessage, { message: "Failed to create routes with meta site URL." });
    },
    FailedToGetUrl: () => {
      return () => /* @__PURE__ */ import_react11.default.createElement(SystemErrorMessage, { message: "Failed to get meta site URL." });
    },
    ...
    FailedToSetCodeIdentifier: () => {
      return () => /* @__PURE__ */ import_react11.default.createElement(ErrorMessage, { cause, message: "Failed to set code identifier." });
    },
    FailedToSetNamespace: () => {
      return () => /* @__PURE__ */ import_react11.default.createElement(ErrorMessage, { cause, message: "Failed to set namespace." });
    },
    FailedToGenerateExtensionConfig: ({ extensionId }) => {
      return () => /* @__PURE__ */ import_react11.default.createElement(
        ErrorMessage,
        {
          cause,
    ...
    ConfigNoDefaultExport: ({ path: path3 }) => {
      return () => /* @__PURE__ */ import_react11.default.createElement(
        ErrorMessage,
        {
          cause,
          message: `No default export in Wix config file at "${path3}"`
        }
      );
    },
...
var Rows = ({ children }) => {
  return /* @__PURE__ */ import_react12.default.createElement(Box_default, { flexDirection: "column", paddingTop: 1, paddingBottom: 1 }, children);
};
var Row = ({ label, value }) => {
  return value ? /* @__PURE__ */ import_react12.default.createElement(Text, null, label, ": ", /* @__PURE__ */ import_react12.default.createElement(Text, { skin: "warning" }, value)) : null;
};
var HttpApplicationErrorDetails = ({ httpError }) => {
  const data = httpError.response?.data;
  const applicationError = data?.details.applicationError;
  return /* @__PURE__ */ import_react12.default.createElement(Rows, null, /* @__PURE__ */ import_react12.default.createElement(Text, null, "Extended error output:"), /* @__PURE__ */ import_react12.default.createElement(Row, { label: "Request ID", value: httpError.requestId }), /* @__PURE__ */ import_react12.default.createElement(Row, { label: "Code", value: applicationError?.code }), /* @__PURE__ */ import_react12.default.createElement(Row, { label: "Message", value: data?.message }), /* @__PURE__ */ import_react12.default.createElement(Row, { label: "Description", value: applicationError?.description }));
};
var HttpValidationErrorDetails = ({ httpError }) => {
  ...
  return /* @__PURE__ */ import_react12.default.createElement(Rows, null, /* @__PURE__ */ import_react12.default.createElement(Text, null, "Extended error output:"), /* @__PURE__ */ import_react12.default.createElement(Row, { label: "Request ID", value: httpError.requestId }), /* @__PURE__ */ import_react12.default.createElement(Row, { label: "Message", value: data?.message }), /* @__PURE__ */ import_react12.default.createElement(Row, { label: "Violations", value: violations }));
};
  ...
  if (httpError) {
    try {
      if (isHttpApplicationError(httpError)) {
        return /* @__PURE__ */ import_react12.default.createElement(HttpApplicationErrorDetails, { httpError });
      }
      if (isHttpValidationError(httpError)) {
        return /* @__PURE__ */ import_react12.default.createElement(HttpValidationErrorDetails, { httpError });
      }
    } catch {
      return null;
    }
...
var ErrorViewer = ({
  ...
  const userFriendlyMessage = (0, import_react13.useMemo)(() => {
    const error = normalizeError(originalError);
    return isCliSystemError(error) && systemErrorOverride ? systemErrorOverride : (0, import_react13.createElement)(getErrorComponent(error.cliCode, error.cause()));
  }, [originalError, systemErrorOverride]);
  return /* @__PURE__ */ import_react13.default.createElement(import_react13.default.Fragment, null, userFriendlyMessage, /* @__PURE__ */ import_react13.default.createElement(HttpErrorDetails, { error: originalError }));
};
...
var _nativeNodeFetchIntegration = (options = {}) => {
  ...
  async function getInstrumentation() {
    ...
    try {
      const pkg = await import("./build-GCRAYCRJ.js");
...
function matchFirst(regex, text) {
  const match12 = regex.exec(text);
  return match12 ? match12[1] : void 0;
}
...
// ../cli-error-reporting/src/error-reporter.ts
var ReportErrorResult = (0, import_variant11.variant)({
  Reported: (0, import_variant11.fields)(),
  Ignored: {}
});
var reportError = (scope, originalError, { data } = {}) => {
  ...
  const error = (0, import_variant11.isType)(originalError.cliCode, CliSystemErrorCode.UnknownFailure) ? originalError.cliCode.error : originalError;
    ...
    captureContext: (localScope) => {
      ...
      if (error instanceof Error) {
        localScope.setContext("Error Details", {
          Info: import_verror2.default.info(error),
          "Full Stack": import_verror2.default.fullStack(error)
        });
      }
...
function createErrorReporter({ dsn, release: release2 }) {
  ...
  return {
    ...
    reportUserFeedback: async ({ userId, eventId, feedback }) => {
      const { BrowserClient } = await import("./esm-FSYXGSHZ.js");
    ...
    setCliSessionId: (cliSessionId) => sentryScope.setTag("cli.session_id", cliSessionId),
    setAiAgentName: (aiAgentName) => {
      sentryScope.setTag("aiagent", "yes");
      sentryScope.setTag("aiagent.name", aiAgentName);
    },
...
var CacheState = (0, import_variant12.variant)({
  Exists: (0, import_variant12.fields)(),
  NotExists: {}
});
...
var ContactUs = () => /* @__PURE__ */ import_react14.default.createElement(
  Trans,
  {
    i18nKey: "user_feedback.contact_us",
    components: [/* @__PURE__ */ import_react14.default.createElement(Link, { url: "https://discord.gg/wixstudio" })]
  }
);
...
var UserFeedbackForm = ({ onSubmit }) => {
  ...
  const [consent, setConsent] = (0, import_react15.useState)();
  const handleConsentSubmit = (0, import_react15.useCallback)((value) => {
    setConsent(value);
    if (!value) {
      setIsSubmitted(true);
    }
  ...
  return /* @__PURE__ */ import_react15.default.createElement(import_react15.default.Fragment, null, /* @__PURE__ */ import_react15.default.createElement(Box_default, { flexDirection: "column", marginTop: 1, marginBottom: 1 }, /* @__PURE__ */ import_react15.default.createElement(Text, null, t("user_feedback.title")), /* @__PURE__ */ import_react15.default.createElement(Text, { skin: "secondary" }, t("user_feedback.subtitle"))), /* @__PURE__ */ import_react15.default.createElement(
    ConfirmInput,
    {
      initialValue: false,
  ...
  ), consent === true && /* @__PURE__ */ import_react15.default.createElement(
    TextInput,
    {
      label: t("user_feedback.comments.label"),
      onSubmit: handleSubmit
    }
  ), isSubmitted && /* @__PURE__ */ import_react15.default.createElement(Box_default, { flexDirection: "column", marginTop: 1 }, /* @__PURE__ */ import_react15.default.createElement(Text, null, consent ? t("user_feedback.submitted") : t("user_feedback.declined")), /* @__PURE__ */ import_react15.default.createElement(ContactUs, null)));
};
...
var UserFeedbackFallback = () => {
  return /* @__PURE__ */ import_react16.default.createElement(Box_default, { flexDirection: "column", marginTop: 1 }, /* @__PURE__ */ import_react16.default.createElement(ContactUs, null));
};
...
async function collectUserFeedback({
  ...
  if (!stdin.isTTY || !cachePath || !(0, import_variant13.isType)(reportResult, ReportErrorResult.Reported)) {
    return;
  }
  ...
  if ((0, import_variant13.isType)(cacheState, CacheState.Exists)) {
    return;
  }
  const user = errorReporter.getUser();
  if (user) {
    await render(
      /* @__PURE__ */ import_react17.default.createElement(I18nProvider, { messages: messages_default2 }, /* @__PURE__ */ import_react17.default.createElement(
        UserFeedbackForm,
        {
          onSubmit: ({ feedback }) => {
    ...
    renderSync(
      /* @__PURE__ */ import_react17.default.createElement(I18nProvider, { messages: messages_default2 }, /* @__PURE__ */ import_react17.default.createElement(UserFeedbackFallback, null))
    );
  }
...
var RawErrorViewer = (props) => {
  ...
  return /* @__PURE__ */ import_react18.default.createElement(Box_default, { flexDirection: "column", paddingTop: 1 }, /* @__PURE__ */ import_react18.default.createElement(Text, null, "----- CLI crash report at ", occurredAt.toISOString()), error instanceof Error ? /* @__PURE__ */ import_react18.default.createElement(FullErrorDetails, { error }) : /* @__PURE__ */ import_react18.default.createElement(Text, null, JSON.stringify(error)));
};
var FullErrorDetails = ({ error }) => {
  const httpError = extractHttpError(error);
  const httpErrorInfo = httpError ? getSafeHttpErrorInfo(httpError) : null;
  return /* @__PURE__ */ import_react18.default.createElement(import_react18.default.Fragment, null, /* @__PURE__ */ import_react18.default.createElement(Text, null, "Error Trace"), /* @__PURE__ */ import_react18.default.createElement(Text, null, import_verror3.default.fullStack(error)), /* @__PURE__ */ import_react18.default.createElement(Text, null, "Error Info"), /* @__PURE__ */ import_react18.default.createElement(Text, null, toPrettyJson(import_verror3.default.info(error))), httpErrorInfo ? /* @__PURE__ */ import_react18.default.createElement(Box_default, { flexDirection: "column" }, /* @__PURE__ */ import_react18.default.createElement(Text, null, "Request Info"), /* @__PURE__ */ import_react18.default.createElement(Text, null, toPrettyJson(httpErrorInfo))) : null);
};
...
function createErrorHandler({
  ...
  const handler = function({ error, data }) {
    ...
    renderErrorSync(
      /* @__PURE__ */ import_react19.default.createElement(import_react19.default.Fragment, null, /* @__PURE__ */ import_react19.default.createElement(ErrorViewer, { error }), isCI && /* @__PURE__ */ import_react19.default.createElement(RawErrorViewer, { error, occurredAt: occuredAt }))
    );
...
var ErrorReporterContext = (0, import_react20.createContext)(null);
var ErrorReporterProvider = (props) => /* @__PURE__ */ import_react20.default.createElement(ErrorReporterContext.Provider, { ...props });
var useErrorReporter = () => {
  const context2 = (0, import_react20.useContext)(ErrorReporterContext);
  if (context2 == null) {
    throw new Error(
...
var LoginInstructions = ({ userCode, verificationUri }) => {
  ...
  const [openBrowserError, setOpenBrowserError] = (0, import_react22.useState)(null);
  ...
  return /* @__PURE__ */ import_react22.default.createElement(import_react22.default.Fragment, null, /* @__PURE__ */ import_react22.default.createElement(Trans, { i18nKey: "login.instructions_title" }), /* @__PURE__ */ import_react22.default.createElement(Box_default, { flexDirection: "column", marginTop: 1, marginBottom: 1, marginLeft: 2 }, /* @__PURE__ */ import_react22.default.createElement(Box_default, { marginBottom: 1 }, /* @__PURE__ */ import_react22.default.createElement(
    Trans,
    {
      i18nKey: "login.instructions_details",
      components: [/* @__PURE__ */ import_react22.default.createElement(Text, { skin: "info" })],
      values: { code: userCode }
    }
  )), /* @__PURE__ */ import_react22.default.createElement(Box_default, { marginBottom: 1 }, /* @__PURE__ */ import_react22.default.createElement(
    Trans,
    {
      i18nKey: "login.have_wix_account",
      components: [
        /* @__PURE__ */ import_react22.default.createElement(Key, { value: "enter" }),
        /* @__PURE__ */ import_react22.default.createElement(Link, { url: coloredVerificationUri })
      ]
    }
    ...
    Trans,
    {
      ...
      components: [/* @__PURE__ */ import_react22.default.createElement(Key, { value: "space" }), /* @__PURE__ */ import_react22.default.createElement(Link, { url: signupUri })]
    }
  )), openBrowserError != null && /* @__PURE__ */ import_react22.default.createElement(import_react22.default.Fragment, null, /* @__PURE__ */ import_react22.default.createElement(Box_default, { flexDirection: "column", marginBottom: 1 }, /* @__PURE__ */ import_react22.default.createElement(ErrorViewer, { error: openBrowserError })), /* @__PURE__ */ import_react22.default.createElement(
    Trans,
    {
      i18nKey: "login.error_opening_browser",
      components: [/* @__PURE__ */ import_react22.default.createElement(Link, { url: coloredVerificationUri })]
    }
  )));
};
...
var Login = ({ loginFn, onLogin, onError }) => {
  const loginMode = useLoginModeContext();
  return loginMode === "agent" ? /* @__PURE__ */ import_react23.default.createElement(AgentLogin, { loginFn, onLogin, onError }) : /* @__PURE__ */ import_react23.default.createElement(InteractiveLogin, { loginFn, onLogin, onError });
};
var AgentLogin = ({ loginFn, onLogin, onError }) => {
  const biLogger = useBiLogger();
  useAsync(
    async ({ signal }) => {
      const authData = await runAgentLoginFlow({
        biLogger,
        loginFn,
        signal
      });
...
var InteractiveLogin = ({ loginFn, onLogin, onError }) => {
  const biLogger = useBiLogger();
  ...
  return (0, import_variant14.match)(status, {
    ...
    Success: ({ result: { userCode, verificationUri } }) => /* @__PURE__ */ import_react23.default.createElement(
      LoginInstructions,
      {
        userCode,
        verificationUri
...
function LoginWithI18n(props) {
  return /* @__PURE__ */ import_react23.default.createElement(I18nProvider, { messages: messages_default }, /* @__PURE__ */ import_react23.default.createElement(Login, { loginFn: unifiedAuthStrategy.login, ...props }));
}
...
var WarnLegacyAuthData = ({ children }) => {
  ...
  return (0, import_variant15.match)(status, {
    Error: () => null,
    Loading: () => null,
    Success: ({ result }) => /* @__PURE__ */ import_react24.default.createElement(import_react24.default.Fragment, null, result && /* @__PURE__ */ import_react24.default.createElement(Box_default, { marginBottom: 1 }, /* @__PURE__ */ import_react24.default.createElement(Trans, { i18nKey: "login.legacy_auth_notice" })), children)
  });
};
...
var EnsureLoggedIn = ({
  ...
  return (0, import_variant16.match)(status, {
    Loading: () => /* @__PURE__ */ import_react25.default.createElement(Spinner, null),
    Error: () => null,
    Success: ({ result: accountAuth }) => {
      if (accountAuth == null) {
        return /* @__PURE__ */ import_react25.default.createElement(I18nProvider, { messages: messages_default }, /* @__PURE__ */ import_react25.default.createElement(WarnLegacyAuthData, null, /* @__PURE__ */ import_react25.default.createElement(
          Login,
          {
            loginFn: login,
            onLogin: (data) => {
...
var AuthProvider = ({ children, onError, onLogin }) => {
  ...
  return /* @__PURE__ */ import_react26.default.createElement(
    ...
    ({ getAccessToken, getRequestHeaders, userInfo }) => /* @__PURE__ */ import_react26.default.createElement(
...
var SiteAuthProvider = ({
  ...
  const exit = useExit();
  const authStrategy = (0, import_react27.useMemo)(() => createSiteAuthStrategy(siteId), [siteId]);
  return /* @__PURE__ */ import_react27.default.createElement(
    ...
    ({ getAccessToken, getRequestHeaders, userInfo }) => /* @__PURE__ */ import_react27.default.createElement(
      AuthContext.Provider,
      {
        value: {
          getAccessToken,
...
var useSiteAuth = ({
  ...
  const exit = useExit();
  const authStrategy = (0, import_react28.useMemo)(() => createSiteAuthStrategy(siteId), [siteId]);
  ...
  if ((0, import_variant17.isType)(status, "Loading")) {
    return {
      isLoading: true,
      isError: false,
  ...
  if ((0, import_variant17.isType)(status, "Error")) {
    return {
      isLoading: false,
      isError: true,
      isAuthenticated: false,
...
function authenticateWithSiteId(siteId, biLogger, errorReporter, loginMode) {
  return new Promise((resolve2, reject) => {
    render(
      /* @__PURE__ */ import_react29.default.createElement(ErrorReporterProvider, { value: errorReporter }, /* @__PURE__ */ import_react29.default.createElement(BiProvider, { value: biLogger }, /* @__PURE__ */ import_react29.default.createElement(LoginModeProvider, { value: loginMode }, /* @__PURE__ */ import_react29.default.createElement(SiteAuthProvider, { siteId }, /* @__PURE__ */ import_react29.default.createElement(Wrapper, { onReady: resolve2 })))))
...
export {
  ...
  emitLoginFlowMessage,
  createAwaitingUserLoginFlowMessage,
  createLoggedInLoginFlowMessage,
  createSuccessfulLoginFlowMessage,
  runAgentLoginFlow,
  LoginModeProvider,
  writeCrashReport,
  DebugLogProvider,
...
//# sourceMappingURL=chunk-IZ2LPIW5.js.map