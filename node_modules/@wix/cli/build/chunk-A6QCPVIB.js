...
// ../cli-astro-commands/src/components/PreviewCommand/hooks/useCreatePreview.ts
init_esm_shims();

// ../cli-astro-commands/src/app-manifest/file.ts
init_esm_shims();
import fs from "node:fs/promises";
async function processAppManifest({
  projectFolder,
  config,
  serverUrl
}) {
  ...
  } = await readBuildMetadata(projectFolder, config);
  if (metaDataSource === "WIX_CONFIG") {
    return {
      appManifest: { appId: config.appId, components: [] },
      componentPlaceholderFieldMasks: []
    };
  }
  ...
  return {
    appManifest: parseLooseAppManifest(manifest, placeholders),
    componentPlaceholderFieldMasks: getComponentPlaceholderFieldMasks(
      manifest,
      placeholders.keys()
    )
  };
...
// ../cli-deployable-files/src/index.ts
init_esm_shims();
...
async function loadDeployableFiles(cwd) {
  return globby("**/*", {
    cwd,
    onlyFiles: true,
    gitignore: true,
    ...
    ignore: [
      ...
      // https://github.com/netlify/cli/blob/8a07280b83de35aae6dee80212d4e065147e3456/src/utils/deploy/upload-source-zip.ts#L23C7-L23C30
      "**/*.log",
      "**/CVS",
      "**/Thumbs.db",
      "**/__pycache__",
...
function useLoadBuildFiles() {
  ...
  return useAsyncCallback(async () => {
    ...
    const load = buildMetadata.metaDataSource === "WIX_CONFIG" ? loadDeployableFiles : loadFiles;
    const client = (buildMetadata.clientDir ? await load(buildMetadata.clientDir) : []).filter(({ path: path2 }) => {
      if (path2 === buildMetadata.appManifestPath) {
        return false;
      }
      if (buildMetadata.serverDir && path2.startsWith(buildMetadata.serverDir)) {
        return false;
      }
      return true;
    });
    const server = (buildMetadata.serverDir ? await load(buildMetadata.serverDir) : []).filter(({ path: path2 }) => {
      if (path2 === buildMetadata.appManifestPath) {
        return false;
      }
      return true;
...
function useDeployFiles() {
  ...
  return useAsyncCallback(async (_, files) => {
    ...
    const hasClientFiles = clientFilesMetadata.length > 0;
    if (!hasClientFiles) {
      const { appDeployment: appDeployment2 } = await backendAsAServiceClient.createAppDeployment(
        config.appId,
        backendFiles,
        void 0,
        buildMetadata.generateAppDeploymentGroupId
      );
      if (!appDeployment2?.id || !appDeployment2.deploymentBaseUrl) {
        throw new CliError({
      ...
      return {
        deploymentId: appDeployment2.id,
        deploymentBaseUrl: appDeployment2.deploymentBaseUrl
      };
    }
    const {
      appDeployment,
    ...
    const clientDir = buildMetadata.clientDir;
    if (clientDir !== void 0 && staticFilesUploadUrls?.[0]) {
...
async function getStaticFileMetadata(file) {
  const stats = await fs2.promises.stat(file.path);
...
function useCreatePreview({
  ...
  const createPreviewIdAction = useAsyncCallback(
    async (_, serverUrl, deploymentId) => {
      ...
      const { appManifest: rawManifest, componentPlaceholderFieldMasks } = await processAppManifest({
        projectFolder: model.projectFolder,
        config: model.config,
        serverUrl
      });
      const appManifest = deploymentId ? updateManifestWithBackendWorker(
        rawManifest,
        deploymentId,
        serverUrl,
        label,
      ...
      if (existingPreviewId) {
        previewId = await devCenterOverrides.updateOverrideWithNoSideEffects({
          id: existingPreviewId,
          appManifest,
          componentFieldMasks: componentPlaceholderFieldMasks,
          projectId: model.releaseAppId
        });
      } else {
        previewId = await devCenterOverrides.createComponentsOverride({
          appManifest,
          projectId: model.releaseAppId,
          experiments: toComponentExperiments(appManifest.components)
        });
      }
      return { previewId, appManifest };
    }
  );
  const { status } = useAsync(async () => {
    await assertBuildOutputExists(model.projectFolder, model.config);
    const { deploymentUrl: previewUrl, deploymentId } = await uploadApplicationAction.execute(baseUrl);
    const { previewId, appManifest } = await createPreviewIdAction.execute(
      previewUrl,
      deploymentId
    );
    const result = { previewId, previewUrl, appManifest };
    onPreviewCreated?.(result);
    return result;
  }, []);
  return {
    uploadStatus: uploadApplicationAction.status,
    createPreviewStatus: createPreviewIdAction.status,
    status
  };
}
...
var SiteTopologyAndLinks = ({ environment, previewId, previewUrl, onTopologyUpdated }) => {
  const { appId, siteId } = useSiteConfig();
  const updateDeploymentTopology = useUpdateDeploymentTopologyForSite(siteId);
  const veloClient = useVeloClient();
  const { status } = useAsync(async () => {
    const [, gitHubOnboarding] = await Promise.all([
      updateDeploymentTopology({
        siteUrl: previewUrl,
        versionOverrideId: previewId,
        environment
      }),
  ...
  return /* @__PURE__ */ import_react3.default.createElement(import_react3.default.Fragment, null, /* @__PURE__ */ import_react3.default.createElement(
    ...
    Error: () => null,
    Loading: () => null,
    Success: ({ result: { gitHubOnboarding } }) => /* @__PURE__ */ import_react3.default.createElement(Box_default, { marginY: 1, flexDirection: "column", gap: 1 }, /* @__PURE__ */ import_react3.default.createElement(Text, null, "Open the preview on:"), /* @__PURE__ */ import_react3.default.createElement(Box_default, { paddingLeft: 2, flexDirection: "column" }, /* @__PURE__ */ import_react3.default.createElement(PlatformLink, { url: previewUrl, platform: "site" }), /* @__PURE__ */ import_react3.default.createElement(
      PlatformLink,
      {
        url: `https://manage.wix.com/dashboard/${siteId}?apps-override=${previewId}`,
        platform: "dashboard"
      }
    ), gitHubOnboarding?.status === OnboardingState.GITHUB_ENABLED && /* @__PURE__ */ import_react3.default.createElement(
      PlatformLink,
      {
        url: `https://vibe.wix.com/projects/${siteId}`,
        platform: "editor"
      }
    )), /* @__PURE__ */ import_react3.default.createElement(LinkClickTip, null))
  }));
};
...
var AppTopologyAndLinks = ({ environment, previewId, onTopologyUpdated }) => {
  const updateDeploymentTopology = useUpdateDeploymentTopologyForApp();
  const { status } = useAsync(async () => {
  ...
  ), (0, import_variant3.match)(status, {
    Error: () => null,
    Loading: () => null,
    Success: () => /* @__PURE__ */ import_react4.default.createElement(Box_default, { marginY: 1, flexDirection: "column", gap: 1 }, /* @__PURE__ */ import_react4.default.createElement(Text, null, "Open the preview on:"), /* @__PURE__ */ import_react4.default.createElement(Box_default, { paddingLeft: 2, flexDirection: "column" }, /* @__PURE__ */ import_react4.default.createElement(RedirectLink, { platform: "site", versionOverrideId: previewId }), /* @__PURE__ */ import_react4.default.createElement(RedirectLink, { platform: "editor", versionOverrideId: previewId }), /* @__PURE__ */ import_react4.default.createElement(
      RedirectLink,
      {
        platform: "dashboard",
        versionOverrideId: previewId
      }
    )), /* @__PURE__ */ import_react4.default.createElement(LinkClickTip, null))
  }));
};

// ../cli-astro-commands/src/components/PreviewCommand/Preview.tsx
var Preview = ({ environment, baseUrl, label, previewId, onPreviewCreated }) => {
  const { model } = useProjectModel();
  const { uploadStatus, createPreviewStatus, status } = useCreatePreview({
    baseUrl,
    label,
    previewId
  });
  ...
  ), (0, import_variant4.match)(status, {
    Error: () => null,
    Loading: () => null,
    Success: ({ result: { previewId: previewId2, previewUrl, appManifest } }) => model.config.projectType === "Site" ? /* @__PURE__ */ import_react5.default.createElement(
      SiteTopologyAndLinks,
      {
        environment,
        previewUrl,
        previewId: previewId2,
        onTopologyUpdated: () => onPreviewCreated?.({ previewId: previewId2, previewUrl, appManifest })
      }
    ) : /* @__PURE__ */ import_react5.default.createElement(
      AppTopologyAndLinks,
      {
        environment,
        previewId: previewId2,
        onTopologyUpdated: () => onPreviewCreated?.({ previewId: previewId2, appManifest })
      }
    )
  }));
};
...
// ../cli-astro-commands/src/components/PreviewCommand/PreviewCommand.tsx
var PreviewCommand = ({ environment, baseUrl, label, previewId, onPreviewCreated }) => {
  const { model } = useProjectModel();
  return model.config.projectType === "Site" ? /* @__PURE__ */ import_react6.default.createElement(SiteAuthProvider, { siteId: model.config.siteId }, /* @__PURE__ */ import_react6.default.createElement(
    Preview,
    {
      baseUrl,
      previewId,
...
//# sourceMappingURL=chunk-A6QCPVIB.js.map