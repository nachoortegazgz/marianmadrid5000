...
describe('Regresiones de modulos', () => {
  ...
  it('normaliza movimientos legacy para construir el payload fiscal', () => {
    ...
    const payload = [
      String(movimiento.previousRecordHash),
      String(movimiento.transactionId),
      String(movimiento.movementType),
      Number(movimiento.accountingAmount),
      String(movimiento.paymentMethod),
    ].join('|');

    expect(payload).toContain(movimiento.previousRecordHash);
    expect(payload).toContain(movimiento.transactionId);
    expect(payload).toContain('50');
    expect(payload).toContain(movimiento.paymentMethod);
  });
});
